...
export default async function HomePage() {
  const services = hasSupabaseEnv()
    ? (await (await createClient()).from("servicos").select("*").eq("ativo", true).limit(6)).data
    : demoServices;
    ...
    <main>
        ...
        <div className="mx-auto grid min-h-[calc(100vh-4rem)] max-w-7xl items-center gap-10 px-4 py-12 sm:px-6 lg:grid-cols-[1.05fr_0.95fr]">
          <div>
            <span className="inline-flex rounded-full border bg-background px-3 py-1 text-sm text-muted-foreground">
              Agenda inteligente para servicos
            </span>
            <h1 className="mt-6 max-w-3xl text-4xl font-bold tracking-tight sm:text-6xl">
              Agendamentos simples, organizados e automatizados.
            </h1>
            <p className="mt-5 max-w-2xl text-lg text-muted-foreground">
              Clientes escolhem servico, profissional, data e horario em poucos cliques. Sua equipe acompanha tudo em um painel seguro com Supabase.
            </p>
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <Link href="/agendar">
                <Button className="w-full sm:w-auto">
                  Agendamento rapido <ArrowRight size={16} />
                </Button>
              </Link>
              <Link href="/admin">
            ...
            <div className="mt-8 grid gap-3 text-sm text-muted-foreground sm:grid-cols-3">
              <span className="flex items-center gap-2"><ShieldCheck size={16} /> RLS e permissoes</span>
              <span className="flex items-center gap-2"><CalendarClock size={16} /> Calendario em tempo real</span>
              <span className="flex items-center gap-2"><CheckCircle2 size={16} /> Confirmacao automatica</span>
            </div>
          </div>
                  ...
                  <div key={hour} className="flex items-center justify-between rounded-lg border bg-background/85 p-4">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        {index % 2 ? "Consulta de avaliacao" : "Atendimento especializado"}
                      </p>
                    </div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-14 sm:px-6">
        <div className="mb-8 flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
          <div>
            <h2 className="text-2xl font-bold">Servicos disponiveis</h2>
            <p className="mt-2 text-muted-foreground">Um catalogo claro para acelerar a decisao do cliente.</p>
          </div>
          <Link href="/agendar">
            <Button variant="secondary">Escolher horario</Button>
          </Link>
        </div>
