...
export async function POST(request: NextRequest) {
  ...
  if (error) {
    if (error.message.includes("Conflito de horario")) {
      return NextResponse.json(
        { error: "Horario ja ocupado para este profissional." },
        { status: 409 }
      );
    }

    if (error.message.includes("Horario fora da disponibilidade")) {
      return NextResponse.json({ error: "Horario fora da disponibilidade" }, { status: 422 });
    }

    if (error.message.includes("passados") || error.message.includes("agendar no passado")) {
      return NextResponse.json({ error: "Nao e permitido agendar no passado" }, { status: 422 });
    }
  ...
  const pushResult = await sendPushToProfessional({
    estabelecimentoId: values.estabelecimento_id,
  ...
  if (!pushResult.sent) {
    console.warn("Falha ao enviar push para profissional", pushResult);
  }
  ...
  return NextResponse.json({ ok: true, push: pushResult }, { status: 201 });
}
